...
const defaultDB = { 
    ...
    settings: { rules: "ברוכים הבאים לפורום!", floatingMessage: { text: "", color: "#f59e0b", id: 0 } }
};
...
function isVeteran(user) { return (user.role === 'admin' || user.role === 'mod' || user.role === 'editor' || (user.totalLikes >= 50)); }

// פונקציות עזר להתראות חכמות
...
app.post('/api/register', async (req, res) => {
    ...
    if (db.users.find(u => u.username === username)) return res.status(400).json({ error: "שם משתמש תפוס." });
    db.users.push({ 
        username, password, role: db.users.length === 0 ? 'admin' : 'user', 
        joinDate: getILTime().split(',')[0], notifications: [], totalLikes: 0, postCount: 0, 
        lastSeen: Date.now(), isApproved: false, restrictedCats: [], requiresApproval: false 
    });
...
app.post('/api/login', async (req, res) => {
    ...
    user.lastSeen = Date.now(); await saveDB(db);
...
app.get('/api/users/info', async (req, res) => { 
    ...
    db.users.forEach(u => { info[u.username] = { role: u.role, joinDate: u.joinDate, totalLikes: u.totalLikes || 0, postCount: u.postCount || 0, avatar: u.avatar, signature: u.signature }; });
    res.json(info);
});
...
app.post('/api/posts', upload.array('attachedFiles', 5), async (req, res) => { 
    ...
    const user = db.users.find(u => u.username === author); if (!user) return res.status(401).json({ error: "משתמש לא נמצא." });
    const newPost = { id: Date.now(), lastUpdated: Date.now(), views: 0, author, title, category, tag, content, date: getILTime(), replies: [], followers: [author], isApproved: true, likes: [], dislikes: [], isLocked: false, isHidden: false }; 
...
app.post('/api/posts/:id/reply', upload.array('attachedFiles', 5), async (req, res) => { 
    ...
    const newReply = { id: Date.now(), author, content, date: getILTime(), likes: [], dislikes: [], isApproved: true, isHidden: false }; 
...
app.post('/api/like', async (req, res) => { 
    ...
    else { target.likes.push(username); if (targetUser) { targetUser.totalLikes++; targetUser.notifications.push({ text: `${username} עשה לייק להודעה שלך!`, threadId: post.id, replyId, isNew: true }); } } 
...
app.post('/api/dislike', async (req, res) => {
    ...
    const idx = target.dislikes.indexOf(username);
    if (idx > -1) { target.dislikes.splice(idx, 1); } else { target.dislikes.push(username); }
    await saveDB(db); res.json({ success: true }); 
});
...
app.post('/api/posts/delete-reply', async (req, res) => { 
    ...
    if (post) { 
        ...
        if (idx !== -1) {
            const author = db.users.find(u => u.username === post.replies[idx].author);
            if(author) author.postCount--;
            post.replies.splice(idx, 1); await saveDB(db); return res.json({ success: true });
        }
    } 
...
app.post('/api/posts/delete-main', async (req, res) => { 
    ...
    if (idx !== -1) { 
        const author = db.users.find(u => u.username === db.posts[idx].author);
        if(author) author.postCount--;
        db.posts.splice(idx, 1); await saveDB(db); return res.json({ success: true });
    } 
    res.status(404).json({ error: "לא נמצא." }); 
});
...
app.post('/api/ping', async (req, res) => {
    ...
    const db = await getDB(); const user = db.users.find(u => u.username === username);
...
app.put('/api/admin/users/:username/role', async (req, res) => { 
    ...
    if (user) { user.role = role; await saveDB(db); res.json({success: true}); }
});
...
app.get('/api/cloud/list/:category', async (req, res) => {
    try { const r = await axios.get(`${VPS_URL}/list/${req.params.category}`, { headers: { 'x-api-key': VPS_API_KEY } }); res.json(r.data); }
...
app.post('/api/cloud/upload/:category', upload.single('file'), async (req, res) => {
    ...
    try {
        const form = new FormData(); form.append('file', fs.createReadStream(req.file.path), req.file.originalname);
        const r = await axios.post(`${VPS_URL}/upload/${req.params.category}`, form, { headers: { ...form.getHeaders(), 'x-api-key': VPS_API_KEY } });
        fs.unlinkSync(req.file.path); res.json(r.data);
...
mongoose.connect(MONGODB_URI).then(() => {
    ...
    app.listen(PORT, () => console.log(`🚀 שרת רץ בפורט ${PORT}`));
