...
const readDB = () => {
    ...
    if (db.posts) {
        db.posts.forEach(post => { if (!post.followers) { post.followers = [post.author]; needsSave = true; } });
    }
...
// פונקציית עזר לבדיקת "משתמש ותיק"
function isVeteran(user) {
    if (user.role === 'admin' || user.role === 'mod') return true;
    return user.veteranProgress >= 10;
}
...
app.get('/api/users/info', (req, res) => {
    ...
    const info = {};
    db.users.forEach(u => { 
        info[u.username] = { role: u.role || 'user', joinDate: u.joinDate || '', totalLikes: u.totalLikes || 0, isVeteran: isVeteran(u) }; 
    });
    res.json(info);
});

// === מחוברים והתראות + בדיקת פעילות ל"ותיק" ===
app.post('/api/ping', (req, res) => {
    ...
    if (username) {
        ...
        if (user) {
            ...
            // בדיקת היעדרות של 5 ימים
            if (now - user.lastActive > FIVE_DAYS) {
                user.veteranProgress = 0; // איפוס התקדמות
            }
            user.lastSeen = now;
            user.lastActive = now;
...
app.get('/api/messages/:username', (req, res) => {
    const db = readDB();
    const msgs = db.messages.filter(m => m.to === req.params.username).reverse();
    // סימון כנקרא
...
app.post('/api/messages', (req, res) => {
    ...
    if (!receiver) return res.status(404).json({ error: "המשתמש אליו ניסית לשלוח אינו קיים." });
    if (!isVeteran(sender)) return res.status(403).json({ error: "רק 'משתמש ותיק' שצבר מספיק לייקים יכול לשלוח הודעות פרטיות." });
...
app.post('/api/posts', upload.single('attachedFile'), (req, res) => {
    ...
    const db = readDB();
    const newPost = { id: Date.now(), author, title, category, content, date: new Date().toLocaleString('he-IL'), fileUrl: req.file ? `/uploads/${req.file.filename}` : null, likes: [], replies: [], followers: [author] };
...
// === ניהול (העברת קטגוריה ועריכה) ===
app.put('/api/posts/move', (req, res) => {
    const { username, postId, newCategory } = req.body;
    const db = readDB();
    const user = db.users.find(u => u.username === username);
    if (!user || user.role !== 'admin') return res.status(403).json({ error: "רק מנהל יכול להעביר אשכולות." });
    
    const post = db.posts.find(p => p.id === postId);
    if (post) { post.category = newCategory; writeDB(db); res.json({ success: true }); }
    else res.status(404).json({ error: "אשכול לא נמצא." });
});
