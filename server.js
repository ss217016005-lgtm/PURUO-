...
// מסד נתונים ברירת מחדל למקרה שאין כלום
const defaultDB = {
    users: [], 
...
// הפונקציה החכמה שקוראת ו*משדרגת* את הנתונים הישנים
const readDB = () => {
    ...
    // שדרוג 1: הוספת מערך דיווחים אם הוא חסר (למניעת קריסה)
    if (!db.reports) {
        db.reports = [];
        needsSave = true;
    }

    // שדרוג 2: הוספת הקטגוריה החדשה אם היא לא קיימת
    if (db.categories && !db.categories.includes("חדשות בציבור")) {
        db.categories.push("חדשות בציבור");
        needsSave = true;
    }

    // שדרוג 3: עדכון המשתמשים הישנים לתצורה החדשה (תפקידים ותאריכים)
    if (db.users) {
        db.users.forEach(user => {
            ...
            if (!user.joinDate) {
                user.joinDate = "משתמש ותיק"; // תאריך רטרואקטיבי למי שכבר נרשם
                needsSave = true;
            }
        });
    }

    // אם המערכת עשתה שדרוג שקט ברקע, היא שומרת את הקובץ המעודכן
    if (needsSave) {
...
app.get('/api/users/info', (req, res) => {
    ...
    const info = {};
    db.users.forEach(u => {
        info[u.username] = { role: u.role || 'user', joinDate: u.joinDate || 'לא ידוע' };
    });
    res.json(info);
});
...
app.post('/api/report', (req, res) => {
    ...
    const db = readDB();
    db.reports.push({
        id: Date.now(), reporter, postId, replyId, reason, date: new Date().toLocaleString('he-IL')
    });
    writeDB(db);
