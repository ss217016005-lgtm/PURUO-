...
// אבטחה: העברת הסיסמאות למשתני סביבה
...
// תיקון השמירה - מניעת ניסיון לעדכן את שדה ה- _id
async function saveDB(dbData) {
    const { _id, ...updateData } = dbData;
    if (_id) {
        await DBModel.updateOne({ _id: _id }, { $set: updateData });
    }
}
...
app.put('/api/posts/edit', async (req, res) => { 
    ...
    // התיקון הקריטי כאן: מוודאים שהאשכול קיים לפני שנגשים לתגובות שלו!
    const post = db.posts.find(p => p.id === postId); 
    if (!post) return res.status(404).json({ error: "לא נמצא" }); 
...
// אבטחה: העברת משתני הענן ל- process.env
// ==========================================
...
async function startServer() {
    try {
        ...
        // חשוב: MONGODB_URI חייב להיות מוגדר נכון!
        await mongoose.connect(MONGODB_URI);
        console.log('✅ מחובר למסד הנתונים בענן');

        // מוודא שהמסד קיים ומוכן לפני שמתחילים לקבל בקשות
        await getDB();
